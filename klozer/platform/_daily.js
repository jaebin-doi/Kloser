renderSidebar('daily');
    ...
    function renderCompetitors() {
      document.getElementById('cpList').innerHTML = competitors.map((c, i) => `
        <li class="flex items-center gap-2 p-2.5 rounded-lg border border-slate-200 bg-white">
          <div class="w-7 h-7 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center font-black text-[.7rem] shrink-0">${c.name.slice(0,1)}</div>
          <div class="flex-1 min-w-0">
            <div class="text-[.82rem] font-bold text-slate-800 truncate">${c.name}</div>
            <div class="text-[.68rem] text-slate-400 truncate">${c.domain}</div>
          </div>
    ...
    function saveSettings() {
      closeSettings();
      showToast('✓ 관심사가 저장되었습니다 · 잠시 후 갱신됩니다');
    }
    ...
    function dlHtml() {
      ...
      const date = todayStr();
      const kwList = keywords.map(k => `<span class="kw">${k}</span>`).join('');
      ...
      const todosHtml = todos.map(t => `<li class="${t.done?'done':''}"><span class="pri pri-${t.priority}"></span>${t.text}<span class="src">[${t.tag} · ${t.source}]</span></li>`).join('');
      const competitors = [
      ...
      const cpHtml = competitors.map(c => `<li><b>${c.name}</b> <span class="tag">${c.tag}</span><br><span>${c.desc}</span></li>`).join('');
...
<section><h2>오늘의 추천 To-Do (${todos.filter(t=>t.done).length}/${todos.length})</h2>
<ul class="todo">${todosHtml}</ul></section>

<section><h2>경쟁사 동향 (최근 24시간)</h2>
<ul class="cp">${cpHtml}</ul></section>

<footer>이 문서는 Kloser 플랫폼에서 자동 생성된 회의 자료입니다 · ${new Date().toLocaleString('ko-KR')}</footer>
</div></body></html>`;
    ...
    function dlWord() {
      ...
      const trendsRows = trends.map(t => `<tr>
        <td><b>${t.kw}</b></td>
      ...
      const todoRows = todos.map((t, i) => `<tr>
...
<h1>오늘의 일 · Kloser</h1>
<div class="meta">${date} · ${new Date().toLocaleString('ko-KR')} 기준 · 영업1팀 김민수 대리</div>
...
<table class="kpi"><tr>
  ...
  <td><div class="label">추천 To-Do</div><div class="value" style="color:#2563eb">${todos.length}</div></td>
  <td><div class="label">긴급 알림</div><div class="value" style="color:#dc2626">3</div></td>
</tr></table>

<h2>모니터링 관심사 (${keywords.length}개)</h2>
<p>${keywords.map(k => `<span class="kw">${k}</span>`).join(' ')}</p>

<h2>시장 트렌드 알림 (지난 7일)</h2>
<table class="data">
...
<h2>오늘의 추천 To-Do (${todos.filter(t=>t.done).length} / ${todos.length} 완료)</h2>
<table class="data">
...
<div class="footer">이 문서는 Kloser 플랫폼에서 자동 생성된 회의 자료입니다 · ${new Date().toLocaleString('ko-KR')}</div>
    ...
    function dlExcel() {
      ...
      trends.forEach(t => trendsAOA.push([t.kw, t.volume, Math.abs(t.change) + '%', t.dir === 'up' ? '상승' : '하락', t.desc]));
      ...
      todos.forEach((t, i) => todoAOA.push([i + 1, t.done ? '✓' : '', t.priority === 'high' ? '높음' : t.priority === 'med' ? '중간' : '낮음', t.text, t.tag, t.source]));
    ...
    // ── 4. PPT 다운로드 (PptxGenJS) ──
    function dlPpt() {
      document.getElementById('dlMenu').classList.add('hidden');
      const pptx = new PptxGenJS();
      pptx.layout = 'LAYOUT_WIDE'; // 13.33 x 7.5"
      pptx.title = 'Kloser 오늘의 일';
      ...
      s.addText('오늘의 일', { x: 0.6, y: 2.4, w: 12, h: 1.2, fontSize: 56, bold: true, color: C.text, fontFace: 'Pretendard' });
      s.addText('시장 트렌드 · 추천 To-Do · 경쟁사 동향', { x: 0.6, y: 3.8, w: 12, h: 0.5, fontSize: 22, color: C.sub, fontFace: 'Pretendard' });
      s.addText(`${todayStr()} · 영업1팀 김민수 대리`, { x: 0.6, y: 6.6, w: 12, h: 0.4, fontSize: 14, color: C.sub, fontFace: 'Pretendard' });
      s.addShape(pptx.ShapeType.line, { x: 0.6, y: 5.8, w: 4, h: 0, line: { color: C.primary, width: 3 } });

      // Slide 2: KPI
      s = pptx.addSlide();
      s.background = { color: C.bg };
      s.addText('오늘의 핵심 지표', { x: 0.6, y: 0.4, w: 12, h: 0.5, fontSize: 24, bold: true, color: C.text, fontFace: 'Pretendard' });
      const kpis = [
        { label: '신규 키워드', value: '128', color: C.text },
        { label: '상승 트렌드', value: '+23', color: C.emerald },
        { label: '추천 To-Do', value: String(todos.length), color: C.primary },
      ...
      s = pptx.addSlide();
      s.background = { color: 'FFFFFF' };
      s.addText('시장 트렌드 알림 (지난 7일)', { x: 0.6, y: 0.4, w: 12, h: 0.5, fontSize: 24, bold: true, color: C.text, fontFace: 'Pretendard' });
      const tRows = [['키워드', '검색량/일', '변화율', '비고'], ...trends.map(t => [t.kw, t.volume.toLocaleString(), (t.dir === 'up' ? '↑ ' : '↓ ') + Math.abs(t.change) + '%', t.desc])];
      s.addTable(tRows, {
      ...
      s = pptx.addSlide();
      s.background = { color: 'FFFFFF' };
      s.addText('오늘의 추천 To-Do', { x: 0.6, y: 0.4, w: 12, h: 0.5, fontSize: 24, bold: true, color: C.text, fontFace: 'Pretendard' });
      todos.forEach((t, i) => {
        const y = 1.3 + i * 0.6;
        if (y > 6.8) return;
      ...
      s = pptx.addSlide();
      s.background = { color: 'FFFFFF' };
      s.addText('경쟁사 동향', { x: 0.6, y: 0.4, w: 12, h: 0.5, fontSize: 24, bold: true, color: C.text, fontFace: 'Pretendard' });
      const cps = [
      ...
      s.addText('AI가 매일 06:00\n시장과 To-Do를 정리합니다', { x: 0.6, y: 2.8, w: 12, h: 1.6, fontSize: 36, bold: true, color: 'FFFFFF', fontFace: 'Pretendard' });
      s.addText('Kloser · AI Sales Assistant', { x: 0.6, y: 6.5, w: 12, h: 0.5, fontSize: 14, color: '94A3B8', fontFace: 'Pretendard' });

      pptx.writeFile({ fileName: fileBaseName() + '.pptx' });
      showToast('✓ PowerPoint 다운로드 완료');
    }
