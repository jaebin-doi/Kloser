...
> **선행 단계**: Phase 0.5 (live stream spike) 완료 — `docs/PHASE_0_5_LIVE_SPIKE.md`, `docs/PHASE_0_5_FINDINGS.md`.
...
- [x] **Step 1** — DB 인프라 (docker-compose + node-pg-migrate + 초기 스키마 + RLS FORCE ENABLE + seed) → `docs/PHASE_1_STEP_1_DB_INFRA.md` · 결과 `docs/PHASE_1_STEP_1_FINDINGS.md`. **코드 완료, runtime 검증 3건 docker 환경 대기**
...
상세 계획: `docs/PHASE_1_STEP_1_DB_INFRA.md`.
...
**산출물**:
- `platform/api.js` — fetch wrapper (`apiGet`, `apiPost`, error normalize, token 처리)
- 임시 dev 로그인 화면 (또는 존재하는 marketing 페이지에 fixture login 추가)
- `platform/live.html` — `kloserWS.connectCallNamespace`가 token을 handshake에 전달
- `server/src/ws/calls.ts` — handshake에서 JWT 검증, `userId`/`orgId` 추출, `SET LOCAL app.org_id` 주입

**완료 기준**:
- 로그인 안 된 상태에서 live.html 열면 redirect 또는 명시적 에러
- 로그인 후 live.html에서 데모 흐름이 그대로 (Phase 0.5 e2e 회귀 가드 통과)
- `__liveSocket` 핸들 제거 또는 dev 플래그 가드
...
- `docs/PHASE_1_STEP_5_FINDINGS.md` (간단)
...
`docs/PHASE_0_5_FINDINGS.md`에서 Phase 1으로 넘긴 항목:

1. **shared types 중복** — Step 4에서 `shared/types/calls.ts` 또는 zod schema 도입
...
- [ ] `docker compose up`으로 postgres + redis가 뜨고 health 정상
- [ ] `npm run migrate:up` → 7개 테이블 + 모든 RLS 정책 적용
- [ ] `npm run seed` → 2개 org seed 완료
- [ ] signup/login/refresh/logout/me 5개 endpoint 200 응답
- [ ] RLS 격리 테스트 통과 (다른 org 데이터 접근 0 rows 또는 에러)
- [ ] viewer가 write endpoint 호출 시 403
- [ ] live.html이 인증된 WS 연결로 데모 흐름 재생 (Phase 0.5 e2e 회귀 통과)
- [ ] Caddy reverse proxy 초안 동작
- [ ] Phase 0.5 인계 7개 항목 모두 처리됨
- [ ] `docs/PHASE_1_STEP_1_FINDINGS.md` ~ `PHASE_1_STEP_5_FINDINGS.md` 작성됨
