...
- **사이드바 프로필이 실제 계정 정보** — 모든 로그인 페이지 사이드바의 상단 카드에는 본인이 속한 조직 이름·플랜 라벨, 하단 버튼에는 본인 이름·한국어 역할 라벨(`관리자` / `매니저` / `직원` / `조회`)이 `/me` 응답으로 채워집니다. 이름이 비어 있으면 이메일의 `@` 앞부분이 fallback. 다른 조직으로 다시 로그인하면 즉시 그 조직 정보로 갱신됩니다.
