...
> **상위 계획**: `docs/PHASE_1_MASTER.md` §3 Step 5.
> **선행**: Step 4 완료 — `docs/PHASE_1_STEP_4_CLIENT_WIRING.md`, `docs/PHASE_1_STEP_4_FINDINGS.md`.
> **기간**: 1일.
...
- [ ] 6. `docs/FASTIFY_GUIDE.md` §8 — `call:start`/`call:chunk`/`call:end` (3 군데) → `start_call`/`text_chunk`/`end_call`로 동기화. JSON 예시의 `"type": "call:chunk"`도 함께. Phase 0.5 인계 항목 마지막 1건 클로즈.
...
- [ ] 9. `docs/PHASE_1_STEP_5_FINDINGS.md` 작성 + 마스터 plan §3 Step 5 체크박스 + Phase 1 전체 §7 게이트 모두 동기화
...
| 2. dev TLS 전략 | **`tls internal`로 self-signed 인증서 발급** (Caddy 내장 CA). 브라우저는 처음 접속 시 경고 — 한 번 트러스트하거나 `--ignore-certificate-errors` 사용 | `cookieSecure=true` 동작 검증을 위해 진짜 HTTPS가 필요. plaintext `:80`만 띄우면 prod-equivalent 검증 안 됨. 자체 CA를 시스템에 install하는 가이드는 별도 (`docs/PHASE_1_STEP_5_FINDINGS.md`) |
...
- `docs/FASTIFY_GUIDE.md` §8 snake_case 동기화 (Phase 0.5 인계 마지막 1건)
- `server/README.md` Caddy 섹션 + 두 모드 비교표
- 루트 `README.md` run section에 Caddy variant 한 단락 + 상태 블록 갱신
- `docs/PHASE_1_STEP_5_FINDINGS.md` + master plan / step plan 체크박스 동기화
...
### 6. `docs/FASTIFY_GUIDE.md` §8 — snake_case 동기화
...
검증: `grep -n "call:" docs/FASTIFY_GUIDE.md` 결과 0건.
...
### 9. `docs/PHASE_1_STEP_5_FINDINGS.md`
...
검증: `grep -n "\[ \]" docs/PHASE_1_MASTER.md` 결과가 의도된 deferral (Phase 2+) 항목만 남았는지 확인.
...
- [ ] `grep -n "call:" docs/FASTIFY_GUIDE.md` 결과 0건
- [ ] `docs/PHASE_1_STEP_5_FINDINGS.md` 작성
