...
- [ ] 9. `test/server/rls_isolation.test.mjs` (`node --test`) — app role 접속, GUC 없음 / GUC=org_a / GUC=org_b 3가지 경로에서 visibility 검증
...
- repository 3개 (organizations / users / memberships) — `getById`, `list` 정도만
- 격리 테스트 (`node --test`) + `npm test` 스크립트
- Phase 0.5 e2e 회귀 통과
...
| 트랜잭션 패턴 | `BEGIN; SET LOCAL app.org_id=$1; ... COMMIT/ROLLBACK` | LOCAL은 트랜잭션 범위라 connection pool 재사용 누수 위험 ↓. `SET app.org_id`는 금지 |
...
| 격리 테스트 러너 | `node --test` (built-in) | dependency 추가 없음 |
| 테스트 파일 위치 | `test/server/rls_isolation.test.mjs` | 기존 `test/` 컨벤션과 일치 |
...
│       ├── db/
│       │   └── pool.ts             # 🟡 prod fallback 제거, app role 풀
│       ├── plugins/
│       │   └── db.ts               # 🆕 fastify plugin: app.pg + app.withOrgContext
│       ├── middleware/
│       │   └── orgContext.ts       # 🆕 preHandler — withOrgContext 래핑
│       └── repositories/
│           ├── organizations.ts    # 🆕 getById, list
│           ├── users.ts            # 🆕 getById, list
│           └── memberships.ts      # 🆕 getById, list
├── test/
│   └── server/
│       └── rls_isolation.test.mjs  # 🆕 node --test
...
### 9. RLS 격리 테스트 (`test/server/rls_isolation.test.mjs`)

`node --test` 사용. seed로 들어간 2개 org를 활용:

- `app` role로 연결
- 케이스 A: GUC 없이 `SELECT FROM memberships` → 0 rows
- 케이스 B: `withOrgContext(org_acme, ...)` 안에서 `SELECT FROM memberships` → 2 rows (acme의 admin+employee), 모두 `org_id = org_acme`
- 케이스 C: `withOrgContext(org_globex, ...)` → 2 rows, `org_id = org_globex`
- 케이스 D: `withOrgContext(org_acme, ...)` 안에서 `INSERT INTO memberships (org_id=org_globex, ...)` 시도 → WITH CHECK 위반 → 에러

각 케이스 독립적인 트랜잭션. 4개 assertion + describe block 1개.

검증:
- `node --test test/server/rls_isolation.test.mjs` → 4/4 PASS

### 10. `npm test` 등록 + e2e 회귀

테스트 파일은 **repo 루트의 `test/server/`**에 둔다 (기존 `test/` 컨벤션 유지). `server/package.json`은 `server/` 디렉토리에서 실행되므로 루트 경로 참조에 주의:

- 옵션 A: `server/package.json`에 `"test": "node --test ../test/server/*.test.mjs"` (server 디렉토리에서 실행, 상대경로 위로)
- 옵션 B: 루트 `package.json` 신설 또는 root 스크립트 (`scripts/test.mjs`)에 통합 entry 두기

본 step에서는 **A** 채택 (root package.json 안 만들고 단순 유지). `npm --prefix server test`로 호출.

기존 `test/phase_0_5_e2e.mjs`도 회귀로 한 번 돌려서 DB 도입이 기존 흐름을 안 깨는지 확인 (Step 1에서 14/14 PASS 기록 있음). 이건 별도 명령 (`node test/phase_0_5_e2e.mjs`).

검증:
- `npm --prefix server test` → 격리 테스트 4/4 PASS
...
- [ ] `node --test test/server/rls_isolation.test.mjs` 4/4 PASS (default-deny + 양방향 scoped + WITH CHECK 위반)
