...
- [x] **Step 7** — Phase 3 e2e + Phase 3 종합 findings → `PHASE_3_STEP_7_E2E.md` + `PHASE_3_STEP_7_FINDINGS.md` (2026-05-11)
...
**`requireFreshRole` 전사 적용** — Phase 3에서는 admin-only mutation (role 변경, status 변경, team 생성/수정/삭제, invitation **생성·재발송·취소**)에만 opt-in 도입. 전 endpoint (read 포함) 적용은 Phase 6+ 검토. password reset / invitation accept 흐름은 익명 토큰 기반이라 본 미들웨어 적용 대상에서 제외 (§2-14 참조).
...
| 12. 마지막 active admin 보호 | **PATCH /memberships/:id에서 role/status 변경 시 트랜잭션 안에서 `SELECT id FROM memberships WHERE org_id = ? AND role = 'admin' AND status = 'active' ORDER BY id FOR UPDATE`로 활성 admin 집합 전체 lock + 변경 시뮬레이션.** 변경 후 활성 admin이 0이 되면 409 + `last_admin_protected` 응답. `FOR UPDATE`를 aggregate `count(*)`에 적용하면 row lock이 안 잡히므로 row를 직접 SELECT/FOR UPDATE 해야 함. `ORDER BY id`로 lock 획득 순서를 결정적으로 — 두 동시 mutator 간 deadlock 방지 | UI 우회로 admin 0 상황 방지. row 집합 lock으로 concurrent mutation 직렬화. 자세한 패턴은 `PHASE_3_STEP_4_TEAM_MEMBER_API.md` §1-3 / §3 |
...
| 14. `requireFreshRole` opt-in | **admin-only mutation 전반에 도입**: role 변경 / status 변경 (Step 4) / team 생성·수정·삭제 (Step 4) / invitation **생성·재발송·취소** (Step 5). 같은 security-class — 막 demote된 admin이 act하는 케이스 모두 차단. 미들웨어가 DB에서 현재 role 재조회 후 JWT 와 비교, 불일치 시 `401 stale_role`. **`POST /auth/password/forgot` / `POST /auth/password/reset` 및 `POST /invitations/accept`는 제외** — 익명 토큰 기반이며 JWT가 첨부되지 않거나 안 쓰이므로 role-stale 검사 자체가 성립하지 않음. password reset / invitation accept의 신뢰 근거는 1회용 sha256 토큰 + TTL + 성공 후 영향 범위 (전 세션 revoke 등)로 충분 | Phase 1 finding §6 / Phase 2 master §7-2 deferred 항목. team mutation도 admin-only이므로 동일 risk-class에 포함하는 게 일관. cost는 endpoint당 DB round-trip 1회 — Phase 3 시점 무시 가능 |
...
- 단위 테스트 — forgot 이메일 enumeration 차단 / reset 토큰 1회 소비 / sessions revoke 후 옛 refresh로 `/auth/refresh` 401 / 만료 토큰 거부
...
- 옛 refresh cookie로 `/auth/refresh` → 401 (session revoked)
- 옛 access token (JWT)는 자연 만료 (`ACCESS_TOKEN_TTL=15m`)까지 유효 — JWT가 stateless라 능동 무효화 안 함. 운영 차원 즉시 무효화는 Phase 6+ session-id cross-check 도입 시점에 검토. 본 step trade-off는 `PHASE_3_STEP_3_PASSWORD_RESET.md` §1-10 / §4에서 확정
...
- `server/src/types/invitation.ts` zod 원본 + `platform/types/invitation.js` JSDoc 사본 + sync registry
...
- `test/phase_3_team_e2e.mjs` — 6~8 시나리오:
  ...
  2. forgot password → reset → 새 비밀번호 로그인 + 옛 refresh로 `/auth/refresh` 401 (세션 revoked). 옛 access token은 자연 만료 (15m TTL)까지 `/me` 200 가능 — Step 3 §1-10 trade-off
...
2. **`requireFreshRole`** — admin-only mutation에 한정 적용: role 변경 / status 변경 / team 생성·수정·삭제 / invitation **생성·재발송·취소**. JWT의 role과 DB의 현재 role 불일치 시 `401 stale_role` (재로그인 안내). **password forgot/reset + invitation accept 흐름은 제외** — 익명 토큰 기반이라 본 미들웨어 적용 대상이 아님 (§2-14 참조)
