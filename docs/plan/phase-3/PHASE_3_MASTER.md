...
- [ ] **Step 4** — Team / Member API + 마지막 admin 보호 + 권한 단위 테스트 → `PHASE_3_STEP_4_TEAM_MEMBER_API.md`
...
**`requireFreshRole` 전사 적용** — Phase 3에서는 admin-only mutation (role 변경, status 변경, team 생성/수정/삭제, invitation 생성/취소)에만 opt-in 도입. 전 endpoint (read 포함) 적용은 Phase 6+ 검토. password reset 흐름은 익명 토큰 기반이라 본 미들웨어 적용 대상에서 제외 (§2-14 참조).
...
| 14. `requireFreshRole` opt-in | **admin-only mutation 전반에 도입**: role 변경 / status 변경 (Step 4) / team 생성·수정·삭제 (Step 4) / invitation 생성·취소 (Step 5). 같은 security-class — 막 demote된 admin이 act하는 케이스 모두 차단. 미들웨어가 DB에서 현재 role 재조회 후 JWT 와 비교, 불일치 시 `401 stale_role`. **`POST /auth/password/forgot` / `POST /auth/password/reset`은 제외** — forgot/reset 흐름은 익명 토큰 기반이며 JWT가 첨부되지 않거나 안 쓰이므로 role-stale 검사 자체가 성립하지 않음. password reset의 신뢰 근거는 1회용 sha256 토큰 + 1h TTL + 성공 후 전 세션 revoke로 충분 | Phase 1 finding §6 / Phase 2 master §7-2 deferred 항목. team mutation도 admin-only이므로 동일 risk-class에 포함하는 게 일관. cost는 endpoint당 DB round-trip 1회 — Phase 3 시점 무시 가능 |
...
2. **`requireFreshRole`** — admin-only mutation에 한정 적용: role 변경 / status 변경 / team 생성·수정·삭제 / invitation 생성·취소. JWT의 role과 DB의 현재 role 불일치 시 `401 stale_role` (재로그인 안내). **password forgot/reset 흐름은 제외** — 익명 토큰 기반이라 본 미들웨어 적용 대상이 아님 (§2-14 참조)
