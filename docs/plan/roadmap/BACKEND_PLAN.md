...
[Fastify API + WebSocket Server]
          ...
          +--> [External APIs]
                 ...
                 - Claude/OpenAI
                 - Naver Search
                 - Resend email delivery (Phase 7 Step 1)
                 - SMTP deferred / optional
                 - HubSpot / Slack
```
...
| Resend email delivery | verify/reset/invite 발송 | 7 | Phase 7 Step 1 implemented. Dev default remains `dev_outbox`; real mode uses transactional `email_outbox` + encrypted sensitive payload + worker delivery. Current source of truth: `docs/plan/phase-7/PHASE_7_MASTER.md` and `docs/plan/phase-7/PHASE_7_STEP_1_FINDINGS.md`; historical design: `PHASE_7_STEP_1_PLAN.md`. |
| SMTP | 메일 발송 대체안 | 8+ | 자체 SMTP는 deliverability/SPF/DKIM/DMARC 운영 부담 때문에 Resend 이후 별도 판단 |
...
### Phase 7 — Operational launch gates

The detailed source of truth for this phase lives under
`docs/plan/phase-7/`. `BACKEND_PLAN.md` is only the roadmap summary.

Step 1 is implemented:

- [x] Resend-capable email delivery path for verify/reset/invite.
- [x] `email_outbox` became a transactional delivery outbox with
      status/provider/retry/dead-letter/lease columns.
- [x] Real-provider rows store redacted archive text plus AES-256-GCM
      encrypted raw URL in `sensitive_payload_*`.
- [x] `EMAIL_PROVIDER=dev_outbox` remains the default and preserves the
      Phase 3 dev/e2e raw URL extraction behavior.
- [x] `EMAIL_PROVIDER=resend` queues pending rows and a BullMQ worker sends
      them through the Resend HTTP adapter after transaction commit.
- [x] Missing/malformed real-provider env is fail-fast. Unknown provider
      values are fail-fast. Error messages do not echo API keys or raw
      tokens.

Remaining Phase 7 gates:

- [ ] Step 2: MFA / session hardening.
- [ ] Step 3: `activity_log` population + audit query surface.
- [ ] Step 4: retention enforce cron, including recovery/sweep policy for
      stuck email `status='sending'` rows.
- [ ] P1: `llm_usage_log.cost_usd_micros` price map, role-based nav
      visibility, report date window / agent drilldown, billing / caps,
      demo-to-real frontend cleanup.
...
2. 이메일 발송 (결정됨)
   - Phase 7 Step 1 provider: Resend.
   - Dev/test default: `EMAIL_PROVIDER=dev_outbox`.
   - Real mode: `EMAIL_PROVIDER=resend` + `EMAIL_FROM` +
     `RESEND_API_KEY` + `EMAIL_OUTBOX_ENCRYPTION_KEY`.
   - 자체 SMTP는 deliverability 운영 부담 때문에 후순위 대체안으로
     남긴다. 뉴스레터 대량 발송은 별도 product surface와 rate-limit
     설계 후 판단한다.
