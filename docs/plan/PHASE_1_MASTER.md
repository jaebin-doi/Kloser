...
- [x] **Step 5** — Reverse proxy (Caddy `tls internal`) + 클라이언트 origin auto-detect + e2e parameterization + FASTIFY_GUIDE snake_case sync → `docs/plan/PHASE_1_STEP_5_REVERSE_PROXY.md` · 결과 `docs/plan/PHASE_1_STEP_5_FINDINGS.md`. **구현 완료** (2026-05-07, 37/37 unit + 16/16 split-origin e2e PASS). **Caddy runtime 검증은 사용자 머신에서 1회 수행 대기** — `caddy validate`, `caddy run` smoke, `KLOSER_E2E_BASE_URL=https://localhost` e2e 16/16. Caddy 바이너리 미설치 환경에서 작성됨.
...
- [ ] Caddy reverse proxy **runtime 검증** — `ops/Caddyfile.dev`는 작성·linted 상태이지만 본 작업 환경에 Caddy 바이너리 미설치. 사용자 머신에서 `caddy validate` + `caddy run` + Caddy variant e2e 16/16 PASS를 1회 확인해야 본 항목 close.
...
**Phase 1 구현 완료** (2026-05-07, 코드·테스트·문서). 위 Caddy runtime 검증 1건만 사용자 머신에서 수행하면 Phase 1 완전 종료. Phase 2 (customers CRUD) 진입은 그 시점부터 권장.
