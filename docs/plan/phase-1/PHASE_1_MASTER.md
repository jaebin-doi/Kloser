...
> **선행 단계**: Phase 0.5 (live stream spike) 완료 — `docs/plan/phase-0.5/PHASE_0_5_LIVE_SPIKE.md`, `docs/plan/phase-0.5/PHASE_0_5_FINDINGS.md`.
...
- [x] **Step 1** — DB 인프라 (docker-compose + node-pg-migrate + 초기 스키마 + RLS FORCE ENABLE + seed) → `docs/plan/phase-1/PHASE_1_STEP_1_DB_INFRA.md` · 결과 `docs/plan/phase-1/PHASE_1_STEP_1_FINDINGS.md`. **완료** (2026-05-07 runtime 검증 통과)
- [x] **Step 2** — DB 연결/저장소 + RLS SET LOCAL + 격리 테스트 → `docs/plan/phase-1/PHASE_1_STEP_2_RLS_CONTEXT.md` · 결과 `docs/plan/phase-1/PHASE_1_STEP_2_FINDINGS.md`. **완료** (2026-05-07, 10/10 격리 + 14/14 e2e PASS)
- [x] **Step 3** — Auth 코어: Argon2id + sessions + signup/login/refresh/logout + GET /me + role middleware → `docs/plan/phase-1/PHASE_1_STEP_3_AUTH_CORE.md` · 결과 `docs/plan/phase-1/PHASE_1_STEP_3_FINDINGS.md`. **완료** (2026-05-07, 29/29 tests + 14/14 e2e PASS)
- [x] **Step 4** — `platform/api.js` + WebSocket handshake auth + `live.html` 통합 → `docs/plan/phase-1/PHASE_1_STEP_4_CLIENT_WIRING.md` · 결과 `docs/plan/phase-1/PHASE_1_STEP_4_FINDINGS.md`. **완료** (2026-05-07, 37/37 unit + 16/16 e2e PASS)
- [x] **Step 5** — Reverse proxy (Caddy `tls internal`) + 클라이언트 origin auto-detect + e2e parameterization + FASTIFY_GUIDE snake_case sync → `docs/plan/phase-1/PHASE_1_STEP_5_REVERSE_PROXY.md` · 결과 `docs/plan/phase-1/PHASE_1_STEP_5_FINDINGS.md`. **완료** (2026-05-07, 37/37 unit + 16/16 split-origin e2e + Caddy single-origin runtime 검증 (`caddy validate`, `curl -k https://localhost/health`, `KLOSER_E2E_BASE_URL=https://localhost` variant e2e 16/16) 모두 PASS).
...
| token 저장 방식 (Step 3 확정) | **access = Authorization Bearer (응답 body, 클라이언트 메모리)** + **refresh = HttpOnly cookie, `Path=/auth`, SameSite=Lax, prod Secure**. desktop도 동일 Bearer 사용. 자세한 근거는 `docs/plan/phase-1/PHASE_1_STEP_3_AUTH_CORE.md` §1·§3. | BACKEND_PLAN §11.1 권장 + WS handshake에서 같은 access token 재사용 |
...
상세 계획: `docs/plan/phase-1/PHASE_1_STEP_1_DB_INFRA.md`.
...
- `docs/plan/phase-1/PHASE_1_STEP_5_FINDINGS.md` (간단)
...
`docs/plan/phase-0.5/PHASE_0_5_FINDINGS.md`에서 Phase 1으로 넘긴 항목:
...
- [x] `docs/plan/phase-1/PHASE_1_STEP_1_FINDINGS.md` ~ `PHASE_1_STEP_5_FINDINGS.md` 작성됨
