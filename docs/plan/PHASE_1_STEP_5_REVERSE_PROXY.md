...
> **상위 계획**: `docs/plan/PHASE_1_MASTER.md` §3 Step 5.
> **선행**: Step 4 완료 — `docs/plan/PHASE_1_STEP_4_CLIENT_WIRING.md`, `docs/plan/PHASE_1_STEP_4_FINDINGS.md`.
> **기간**: 1일.
...
| 2. dev TLS 전략 | **`tls internal`로 self-signed 인증서 발급** (Caddy 내장 CA). 브라우저는 처음 접속 시 경고 — 한 번 트러스트하거나 `--ignore-certificate-errors` 사용 | `cookieSecure=true` 동작 검증을 위해 진짜 HTTPS가 필요. plaintext `:80`만 띄우면 prod-equivalent 검증 안 됨. 자체 CA를 시스템에 install하는 가이드는 별도 (`docs/plan/PHASE_1_STEP_5_FINDINGS.md`) |
...
- `docs/plan/PHASE_1_STEP_5_FINDINGS.md` + master plan / step plan 체크박스 동기화
...
### 9. `docs/plan/PHASE_1_STEP_5_FINDINGS.md`
...
검증: `grep -n "\[ \]" docs/plan/PHASE_1_MASTER.md` 결과가 의도된 deferral (Phase 2+) 항목만 남았는지 확인.
