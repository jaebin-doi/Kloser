...
> **상위 계획**: `docs/plan/roadmap/BACKEND_PLAN.md` v0.4 §8 Phase 2.
> **선행 단계**: Phase 1 완료 — `docs/plan/phase-1/PHASE_1_MASTER.md`.
...
- [x] **Step 1** — `customers` 스키마 + RLS 정책 + 시드 → `docs/plan/phase-2/PHASE_2_STEP_1_SCHEMA.md`
- [x] **Step 2** — Repository + service + RLS 격리 단위 테스트 → `docs/plan/phase-2/PHASE_2_STEP_2_REPO.md`
- [x] **Step 3** — Shared types 도입 + zod validation 패턴 (Phase 0.5 인계 deferred 처리) — `server/src/types/customers.ts` source-of-truth + `platform/types/customers.js` JSDoc 사본 + sync 테스트. **`zod` 의존성 추가** → `docs/plan/phase-2/PHASE_2_STEP_3_SHARED_TYPES.md`
- [x] **Step 4** — REST routes (6 endpoints) + role 미들웨어 통합 + zod schema import + route 테스트 → `docs/plan/phase-2/PHASE_2_STEP_4_ROUTES.md`
- [x] **Step 5** — `platform/customers.html` 실 API 연결 + new customer 모달 + 검색·필터·정렬 + domain cleanup (`customers.plan` 제거) → `docs/plan/phase-2/PHASE_2_STEP_5_CLIENT.md`, `docs/plan/phase-2/PHASE_2_STEP_5_FINDINGS.md`
- [x] **Step 6** — Customers e2e + findings + master plan 동기화 → `docs/plan/phase-2/PHASE_2_STEP_6_E2E.md`, `docs/plan/phase-2/PHASE_2_STEP_6_FINDINGS.md`

---

## 0. 왜 Phase 2인가

Phase 1은 **인증·격리·권한 기반**을 만들었다. 그 위에 무엇이든 올릴 수 있게 됐지만 **아직 실제 비즈니스 데이터는 하나도 없다**. Phase 2는 그 위에 첫 비즈니스 entity를 얹는다 — **customers** (Kloser 사용사가 관리하는 영업 대상 고객/리드).

> `customers`는 **테넌트(=organization)가 보유한 영업 대상 (leads/contacts)**이다. Kloser 자체의 구독 플랜 (`organizations.plan` = Starter/Pro/Enterprise)과는 별개의 도메인이며, customer row는 Kloser 플랜 attribute를 가지지 않는다 (Phase 2 Step 5 findings — domain cleanup record).
...
- 인덱스 (검색 ILIKE + 필터 status + soft delete 제외 partial index)
...
| 6. plan 형식 | ~~text NULLABLE + CHECK~~ — **컬럼 자체 제거** (Step 5 findings 도메인 정리). `customers.plan`이 `organizations.plan` (Kloser 구독 플랜)과 의미가 충돌해 멀티테넌트 도메인 경계가 섞였음. 새 마이그레이션 `1715000003000_drop_customers_plan.sql`이 컬럼·partial 인덱스 모두 drop. customer 라이프사이클 stage가 필요해지면 별도 컬럼명 (`stage` 등)으로 재도입 | 도메인 분리. `organizations.plan`은 Kloser 자체 구독 단계, customer row는 영업 대상 자체의 속성만 가진다 |
...
| 12. 검색·필터 query string | `?q=&status=&assigned=me&offset=&limit=&sort=&dir=` — URL이 곧 view state. (`plan` query는 Step 5 도메인 정리에서 제거됨) | 브라우저 뒤로 가기 / 북마크 가능. mock UI의 `activeFilter` state를 URL로 승격 |
...
- `docs/plan/phase-2/PHASE_2_STEP_1_SCHEMA.md` — 컬럼·정책·인덱스 사전 결정
- `docs/plan/phase-2/PHASE_2_STEP_1_FINDINGS.md` — 결과 인계
...
- `docs/plan/phase-2/PHASE_2_STEP_3_SHARED_TYPES.md` — 패턴 문서: server source-of-truth → JSDoc 사본 → sync 검증의 절차. Phase 3+ 새 entity 추가 시 동일 단계 follow.
...
- `docs/plan/phase-2/PHASE_2_STEP_6_E2E.md` (실행 절차)
- `docs/plan/phase-2/PHASE_2_STEP_6_FINDINGS.md` (Phase 2 종합 인계)
...
> 본 모델은 Step 5 findings의 domain cleanup record가 적용된 **최종 형태**다. 1715000002000_customers.sql이 처음 도입할 때는 `plan text` 컬럼과 `customers_org_plan_idx`가 있었지만, 1715000003000_drop_customers_plan.sql이 둘 다 drop했다. Step 1 plan/findings 문서들은 도입 시점 컬럼을 명시하지만 아래 SQL이 master 기준이다.
...
CREATE TABLE customers (
  ...
  -- classification (lifecycle stage; Kloser subscription tier lives on
  -- organizations.plan and is NOT a customer attribute)
...
- [x] `npm --prefix server run typecheck` PASS (`zod` import 포함)
- [x] `npm --prefix server test` PASS — **65/65** (auth 19 + customers_repo 12 + customers_routes 16 + orgContext 3 + rls 7 + ws_auth 8)
- [x] `node test/phase_0_5_e2e.mjs` 16/16 회귀 PASS (split-origin)
- [x] `node test/phase_2_customers_e2e.mjs` 7 시나리오 + cleanup 검증 PASS
- [x] `node test/sync_shared_types.mjs` PASS (server source-of-truth ↔ browser JSDoc 사본 정합)
- [x] `server/package.json`에 `zod` runtime dependency 추가됨 + `package-lock.json` commit
- [x] `customers` 테이블 + `current_app_org_id()` 기반 4 RLS 정책 + 5 인덱스 + 24명 seed 적용됨 (도입 시점 6 인덱스 — `customers_org_plan_idx`는 1715000003000_drop_customers_plan.sql에서 drop)
- [x] 6개 customer endpoint (`GET /customers`, `GET /customers/stats`, `GET /customers/:id`, `POST`, `PATCH`, `DELETE`) 모두 200/4xx 정확히 응답
- [x] viewer JWT로 POST/PATCH/DELETE 호출 시 403 (server unit test customers_routes case 16)
- [x] 다른 org JWT로 customer 접근 시 본인 org 데이터만 보임 (RLS 격리 — server unit + 본 step e2e 시나리오 5 모두 통과)
- [x] `platform/customers.html`이 mock 데이터 사용 안 함 — 실 API 호출만
- [x] new customer 모달이 실작동 (POST + `loadAll()` 재조회로 목록 갱신, 본 step e2e 시나리오 2)
- [x] Shared types 패턴 `sync_shared_types.mjs` 통과
- [x] `docs/plan/phase-2/PHASE_2_STEP_1_FINDINGS.md` ~ `PHASE_2_STEP_6_FINDINGS.md` 작성됨
- [x] 루트 `README.md` + `server/README.md` 상태 블록 Phase 2 완료로 갱신
