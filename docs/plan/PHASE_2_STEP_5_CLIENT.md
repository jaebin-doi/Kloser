...
- [ ] 1. 페이지네이션·렌더 정책 사전 결정 (본 plan §2) 검증
- [ ] 2. 모달 듀얼 모드 (create + edit) 사전 결정 (본 plan §6) 검증
- [ ] 3. ISO Date → 한국어 상대시간 helper 위치 사전 결정 (본 plan §7) 검증
- [ ] 4. 에러 응답 두 형식 분기 정책 사전 결정 (본 plan §8) 검증
- [ ] 5. URL query string 동기화 정책 사전 결정 (본 plan §5) 검증
- [ ] 6. `platform/api.js`에 `apiPatch` + `apiDelete` 추가
- [ ] 7. `platform/_shared.js`에 ISO → 상대시간 helper (`formatRelativeTime`) 추가
- [ ] 8. `platform/customers.html` 갱신 — mock JS 제거 + fetch 흐름 + 모달 듀얼 모드 + URL sync + 에러 처리
- [ ] 9. 브라우저 시각 검증 — Acme 12명 / 신규 추가 / 수정 / 삭제 / Beta 격리 5 시나리오 (본 plan §9)
- [ ] 10. `node test/sync_shared_types.mjs` PASS (회귀)
- [ ] 11. `npm --prefix server test` 65/65 회귀 PASS (서버 무변경)
- [ ] 12. `node test/phase_0_5_e2e.mjs` 16/16 회귀 PASS
- [ ] 13. `docs/plan/PHASE_2_STEP_5_FINDINGS.md` 작성 (구현 + 검증 후 별도 커밋)
...
- 검색 input + 필터 chip 3 그룹 (status / plan / sort 동시 active 가능) → URL query string → `GET /customers?...` 호출 → 렌더
...
| 3. 필터 chip 구조 — **3 그룹 분리** (status / plan / sort) | mock UI는 단일 `.filter-chip` 그룹 (한 번에 하나만 active). Step 5는 3 그룹으로 재구성해 status/plan/sort를 동시에 URL query에 올림. (a) **status 그룹**: 전체 / 활성 / 검토중 / 대기 → `status=`/`active`/`review`/`pending`. (b) **plan 그룹**: 전체 / Enterprise / Pro / Starter → `plan=`/`Enterprise`/`Pro`/`Starter`. (c) **sort 그룹**: 최신순 / 최근 연락 → `sort=created_at` (default) / `sort=last_contacted_at&dir=desc`. 각 그룹은 자체 단일 선택 (그룹 내 chip 클릭 시 같은 그룹 chip에서만 active 제거). 그룹 간 동시 active 가능 → 예: `?status=active&plan=Enterprise&sort=last_contacted_at&dir=desc` | 서버 query 계약 (`CustomerListQuery`)이 status/plan/sort 3 차원을 별개로 받음 — 단일 그룹은 한 차원만 활용해 계약 미사용. 사전 결정 §2-12 (URL이 view state)도 다차원 chip을 전제로 함 |
...
| 10. POST/PATCH/DELETE 후 처리 | 응답 검증 후 **`await loadAll()`로 list + stats 서버 재조회**. local prepend/in-place/splice + KPI 직접 +/- 같은 optimistic update는 본 step에서 **하지 않음** | URL query (q/status/plan/sort)가 활성 상태에서 local-only 반영은 정합 깨짐 — 예: `status=active` 화면에서 `pending`으로 POST하면 prepend된 row가 필터 위반, `sort=last_contacted_at` 상태에서 prepend는 정렬 위반, PATCH로 status/plan/name 바뀌면 현재 view에서 빠져야 할 row가 남거나 보여야 할 row가 안 보임. limit=100 단일 fetch는 데모 규모에서 ms 단위 — 정합성 가치 >> 부하. optimistic update는 Phase 6+ 운영 최적화 시점 |
...
| 15. POST 모달 필드 vs 서버 schema | mock 모달은 name/company/email/phone/status + memo. memo는 서버 schema에 **없음** — 모달에서 제거 (또는 비활성 + "추후 지원" 안내). plan은 NULL 기본 (마스터 §1) — 모달에 plan 선택 추가도 검토 | mock UI의 memo 필드는 본 phase 미구현. 제거가 깔끔 — 서버 schema와 정합. plan은 모달에 추가 (Starter/Pro/Enterprise/-) |
...
<div class="chip-group" data-group="plan">
  <button class="filter-chip active" data-value="">전체 플랜</button>
  <button class="filter-chip" data-value="Enterprise">Enterprise</button>
  <button class="filter-chip" data-value="Pro">Pro</button>
  <button class="filter-chip" data-value="Starter">Starter</button>
</div>
...
document.querySelectorAll('.chip-group').forEach((group) => {
  const dim = group.dataset.group; // 'status' | 'plan' | 'sort'
...
// 2. 상태 (URL ↔ 메모리)
let viewState = readViewStateFromUrl();   // { q, status, plan, sort, dir }
...
- `renderTable()`: server `Customer` shape 그대로 사용. `c.last_contacted_at`은 ISO string → `formatRelativeTime(c.last_contacted_at)`로 변환. `c.plan` null이면 `-` 표시. `c.company` null이면 빈칸
...
```
?q=<search>&status=<active|review|pending>&plan=Enterprise&sort=last_contacted_at&dir=desc
```
...
function readViewStateFromUrl() {
  ...
  return {
    ...
    plan: p.get('plan') || undefined,         // Starter|Pro|Enterprise
...
| 플랜 | select (-/Starter/Pro/Enterprise) | `plan` (NULL = `-`) | "-" 선택 시 `value=""` → JS에서 `null` 매핑 |
...
| 모달 필드 갯수 vs 서버 schema 차이 | mock의 `memo` 제거, `plan` 추가 (사전 결정 §2-15). 서버 `CustomerCreateInput` schema와 정확 정합 |
...
| edit 모달의 plan select 빈값 처리 | "-" 표시 = 서버 NULL. `<option value="">-</option>` + JS에서 `value === '' ? null : value` 매핑 |
...
- [ ] `platform/api.js`에 `apiPatch`, `apiDelete` 추가, `window.kloserApi`에 export
- [ ] `platform/_shared.js`에 `formatRelativeTime(iso)` 추가, `window.formatRelativeTime`에 export
- [ ] `platform/customers.html`에서 mock JS (12명 배열, 하드코딩 KPI 숫자) 모두 제거
- [ ] `platform/customers.html`이 `<script src="api.js"></script>`를 포함 (`_shared.js` 보다 먼저)
- [ ] **필터 chip 3 그룹 분리** (status / plan / sort) — `<div class="chip-group" data-group="...">` 컨테이너 단위로 active toggle. status·plan·sort 동시 활성 → URL `?status=&plan=&sort=&dir=` 다차원 매핑
- [ ] **모달 필수 검증 = `name`만** — mock의 email 필수 표식 (`<span class="text-rose-500">*</span>`) + `addCustomer`의 `if (!name || !email)` 모두 제거. email/phone/company는 빈값 허용, 서버에 null 또는 누락 전달. memo textarea도 제거 (서버 schema 없음)
- [ ] 페이지 로드 시 `GET /customers + GET /customers/stats` 병렬 호출 + 12명 + 4 KPI 정확히 표시 (admin@acme.test)
- [ ] 검색 input + 필터 chip + 정렬 chip 모두 URL query string에 반영, `replaceState`로 새로고침 시 view 복구
- [ ] "고객 추가" 모달 → POST → 201 → **`loadAll()` 재호출** → 현재 URL view + 서버 stats 기준 갱신
- [ ] 행 클릭 → edit 모달 (값 prefill) → PATCH → 200 → **`loadAll()` 재호출** → 동일 갱신
- [ ] edit 모달의 "삭제" → confirm → DELETE → 204 → **`loadAll()` 재호출** → 동일 갱신
- [ ] **필터 정합** — `status=active` chip 활성 상태에서 `pending` 고객을 POST해도 list 그대로 (필터 위반 없음). KPI 카드만 서버 stats 기준 +1 반영
- [ ] 다른 org (admin@beta.test) 로그인 시 다른 12명 + 다른 KPI 분포 (격리 시각)
- [ ] 에러 응답 두 형식 (`invalid_<field>` vs `invalid_input`) 모두 사용자 메시지로 표시 (Korean)
- [ ] `formatRelativeTime` 출력이 mock UI 외관 ("2시간 전", "어제", "3일 전") 매치
- [ ] `node test/sync_shared_types.mjs` PASS
- [ ] `npm --prefix server test` 65/65 회귀 PASS
- [ ] `node test/phase_0_5_e2e.mjs` 16/16 회귀 PASS
- [ ] `docs/plan/PHASE_2_STEP_5_FINDINGS.md` 작성 (구현 + 검증 후 별도 커밋)
