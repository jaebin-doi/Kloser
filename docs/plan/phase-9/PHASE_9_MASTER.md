...
상위 문서: `docs/plan/phase-8/PHASE_8_CLOSEOUT_FINDINGS.md`, `docs/plan/roadmap/DESKTOP_APP_PLAN.md`, `docs/product/realtime-call-assistant-guide.md`
...
- [ ] **Step 1 - desktop capture architecture**: Windows audio engine, Flutter UI shell, backend realtime ingest contract, call lifecycle, security boundary 확정.
- [ ] **Step 2 - backend audio ingest contract**: `/calls` WebSocket에 binary/audio chunk path 또는 별도 namespace 확정, STT adapter input contract와 usage envelope 정리.
...
- **오디오 API**: WASAPI loopback + microphone capture. 구현 라이브러리는 Step 1에서 `NAudio`, `CSCore`, 직접 WASAPI wrapper 중 비교.
- **UI**:
  - 기본 판단: Flutter Windows는 UI shell로 가능.
  - 단, audio capture 자체는 Dart/Flutter plugin에 맡기지 않는다. C# audio engine을 별도 process/library로 두고 Flutter는 local IPC로 제어한다.
  - 빠른 PoC는 C# console/tray/WPF로 먼저 검증해도 된다. UI polish는 Flutter로 분리 가능.
...
- Preserve source labels in the wire envelope even if backend initially mixes. This avoids losing the agent/customer split later.
...
But for production, binary WebSocket frames are better than base64 JSON.

Planned progression:

1. Step 2: define JSON metadata + binary payload framing.
2. Step 3 PoC: allow base64 JSON only if it speeds up capture validation.
3. Step 5 integration: switch to binary frame if base64 overhead is measurable.
...
Current STT adapter supports mock fixture strings and real provider buffer path exists conceptually.

Phase 9 backend must define:

- accepted audio format for adapter input.
- chunk aggregation window:
  - e.g. 1~5 seconds before provider STT call.
...
- extend provider usage metadata with `audio_duration_ms`.
...
- accept authenticated `audio_start` / `audio_chunk` / `audio_end`.
- validate metadata.
- do not call real STT yet unless mock path can consume deterministic fixture/buffer.
- append transcript from mock STT in deterministic test path.
...
These must be answered in Step 1 before implementation.

1. C# audio library: `NAudio`, `CSCore`, or direct WASAPI wrapper?
2. UI first shell: Flutter Windows or C# WPF/tray for PoC?
3. Desktop/backend IPC if Flutter is used: named pipe, gRPC named pipe, or localhost loopback?
4. Audio chunk transport: JSON+base64 first or binary WebSocket from day one?
5. STT input window: 1s, 3s, or 5s segments?
6. Source policy: mix mic+system before backend, or source-separated chunks?
7. Final archive format: WAV, WebM/Opus, Ogg/Opus, or MP4/AAC?
8. Local retry policy for failed archive upload.
9. Device identity model.
10. Legal consent placeholder copy.
...
Next task should be **Phase 9 Step 1 - Desktop Capture Architecture Plan**.

Recommended handoff:

```text
Phase 9 Step 1 계획서를 작성한다. 구현은 하지 않는다.
...
Step 1에서 확정할 것:
- C# audio library 후보 비교: NAudio / CSCore / direct WASAPI wrapper.
- Flutter UI shell vs C# WPF/tray PoC 선택 기준.
- Flutter 사용 시 C# engine과의 IPC 방식.
- WebSocket audio chunk wire contract.
- mic + system audio source separation/mix policy.
- STT chunk aggregation window.
- final recording archive format과 Phase 8 upload/finalize 연결 방식.
- auth/token storage/security policy.
- Windows pilot device matrix.

산출물:
- docs/plan/phase-9/PHASE_9_STEP_1_DESKTOP_CAPTURE_ARCHITECTURE.md
- docs/plan/phase-9/PHASE_9_MASTER.md의 Step 1 entry 업데이트
