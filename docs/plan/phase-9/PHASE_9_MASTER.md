...
- [x] **Step 6 - recording archive bridge**: 실시간 통화 종료 직후 desktop이 stereo WAV (left=agent_mic / right=system_loopback / pcm_s16le_16000) 를 만들어 Phase 8 `POST /calls/:id/recordings/upload` → signed `PUT /dev-recordings/...` → `POST .../finalize` 흐름으로 call에 첨부. Manual E2E PASS (`call_recordings.status='available'`, `calls.html` 재생). dev-only local provider PUT/GET handler (`server/src/routes/devRecordingStorage.ts`)는 NODE_ENV+provider 자기-게이트. 정본 계획 `PHASE_9_STEP_6_PLAN.md`, 결과 `PHASE_9_STEP_6_FINDINGS.md`.
...
Status: complete. See `PHASE_9_STEP_2_FINDINGS.md`.
