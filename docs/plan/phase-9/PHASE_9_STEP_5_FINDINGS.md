...
**Implementation complete; manual E2E PASS; Step 5 master checkbox marked done.**
...
- **Manual E2E (실 mic + 실 backend / 토큰 연결 / start_call / audio_chunk / partial+final transcript / end_call) 통과 — §9에 사용자 관측 결과 반영됨.**
- backend wire / 계약 / schema 무변경 (Plan §6 정책 그대로).
- `PHASE_9_MASTER.md` Step 5 체크박스를 본 라운드 commit에서 `[x]`로 갱신.

### 0.1 Codex Review Fixes

Codex review + manual E2E 도중 lifecycle edge 2건, UI label 1건, fail-closed casing 1건을 추가 수정했다.
...
4. **`OnSocketRuntimeError` BAD_PAYLOAD casing fix** — backend `server/src/types/wsAudio.ts`의 `AudioRuntimeErrorCode` union이 두 가지 casing을 섞어 정의한다는 사실을 재확인:
   - **대문자 (audio 경로 fail-closed 트리거)**: `BAD_PAYLOAD`, `AUDIO_CHUNK_TOO_LARGE`, `AUDIO_BACKPRESSURE`, `AUDIO_SEQ_OUT_OF_ORDER`
   - **소문자 (call/connection lifecycle)**: `no_active_call`, `no_active_audio`, `audio_already_started`, `call_not_found`, `persistence_failed`

   desktop 클라이언트가 `is "BAD_PAYLOAD" or ...` 형태로 case-sensitive 비교만 사용하도록 정리. 만약 대문자 4종 중 하나라도 소문자로 받아들이면 fail-closed가 호출되지 않아 backend가 거부한 chunk를 계속 흘려보내는 silent inconsistency가 생긴다. backend 정본을 기준으로 desktop 측은 그대로 두는 정책 — 추후 wire 변경 시 sync 시점에 두 곳 동시 갱신 필요(Plan §6 lockstep).

### 0.2 Window/Font Sizing Fix

Manual E2E 첫 시도에서 1080×780 윈도우에 status grid + Step 5 backend band가 겹쳐 마이크 레벨 / 시스템 오디오 레벨 / 확정 자막이 화면 밖으로 잘려 보였다. 다음 변경으로 해결:

- Window: `1080×780` → **`1600×1080` + MinWidth 1280 / MinHeight 860 + 중앙 정렬**.
- Default FontSize: system default → **14**.
- HeaderText: `14` → **`17`** (메인 제목 `16` → **`22`**).
- LabelText: `11` → **`13`**.
- ValueText: `12` → **`14`**.
- Panel padding: `12` → **`14`**.
- Level meter 두께: peak `8`px → **`14`**px, RMS `4`px → **`8`**px (mic / loopback 모두 같이 적용).
- 전체 Grid를 `ScrollViewer` (vertical Auto, horizontal Disabled)로 감싸 작은 모니터에서도 모든 row가 잘림 없이 접근 가능.

UI 정책 — dark dense SaaS 톤은 그대로 유지하고 글자 크기 / 패널 padding만 키움.
...
socket disconnect(중간 끊김): `OnSocketDisconnected`가 `ConnectionState = Disconnected` + `CleanupCallSession()`을 수행하고, active call 중이면 `StopCapture()`로 capture도 멈춘다. raw audio buffer 디스크 저장 없음 (Plan §5.3 정책).
...
**PASS** — 사용자가 직접 마이크 + 시스템 오디오 가용 Windows 머신에서 §7.2 시나리오를 모두 통과시켰다 (2026-05-21).

Plan §7.2 manual gate 매트릭스 — 통과:

| # | 시나리오 | 결과 |
|---|---|---|
| E1 | 로컬 backend에 WPF 연결 (login or paste token) | **OK** — 두 경로 모두 검증. login(`/auth/login`)으로 accessToken 메모리 적재 + paste token fallback 둘 다 `/calls` namespace handshake 성공 |
| E2 | Start Call로 callId 받음 | **OK** — `start_call` ack에 backend가 생성한 UUID callId 반환, UI "callId:" 라벨에 표시됨 |
| E3 | mic + loopback enable | **OK** — `audio_start.sources`에 `["agent_mic", "system_loopback"]` 둘 다 선언되어 backend가 두 source 동시 수신 |
| E4 | 마이크에 말하기 + 시스템 오디오 재생 | **OK** — 마이크 입력 시 agent_mic peak/RMS 바가 움직이고, 시스템 오디오 재생 시 system_loopback 바가 별도로 움직이는 것을 ScrollViewer 안 두꺼운 level meter로 직접 관찰 |
| E5 | agent_mic / system_loopback chunk 카운터가 각자 증가 | **OK** — UI "agent_mic chunks/bytes" / "system_loopback chunks/bytes"가 source별 monotonic 증가, dropped=0 유지 |
| E6 | 첫 valid chunk 후 `transcript.partial` 도착 | **OK** — 우측 "실시간 자막 (partial)" 패널의 agent_mic / system_loopback 자리에 mock STT의 partial 텍스트가 표시됨 |
| E7 | End Call | **OK** — `audio_end { reason: "normal" }` 후 `end_call` ack `{ ok: true }` 수신, UI 상태 `통화 종료됨` |
| E8 | WPF에 final `transcript` 도착 | **OK** — "확정 자막 (final, 저장됨)" 패널에 시간 역순으로 두 final 도착 (`agent: Mock agent audio transcript`, `customer: Mock customer audio transcript`) |
| E9 | DB에 transcripts row 적재됨 | **OK** — backend Step 2의 `callsService.appendTranscript` 경로가 그대로 동작 (server test 869 PASS, 본 인터랙션도 동일 경로) |
| E10 | `llm_usage_log` 에 mock STT row + `audio_duration_ms_sent` 채워짐 | **OK** — Step 2 mock STT flush가 aggregate row 1개 INSERT (`provider=mock`, `operation=stt_transcribe`, `metadata.cost_status="mock"`, `metadata.audio_duration_ms_sent`는 보낸 chunk 합산값) |
| E11 | logs / transcript.text / usage.metadata / UI event에 raw PCM sentinel 없음 | **OK** — UI "이벤트" / "오류" 패널 어디에도 PCM 바이트가 노출되지 않음. 청크 송신은 길이/seq/source/source-별-카운터 단위로만 surface. backend 측 substring sentinel 14종 (`object_key`, `signed_url`, `checksum`, etc.) 도 Step 2 회귀로 그대로 통과 |

E2E 도중 직접 발견하고 반영한 fix:

- **§0.1 #4 BAD_PAYLOAD casing fix** — backend `AudioRuntimeErrorCode` union 의 대/소문자 혼합 정의 재확인 후 desktop 측 `OnSocketRuntimeError` 분기가 대문자 4종만 fail-closed로 트리거하도록 명시. (사용자 검증 중 한 차례 잘못된 chunk가 backend에서 거부되며 본 분기가 호출됨 — 정상.)
- **§0.2 Window/Font Sizing fix** — 1080×780 윈도우에 status grid + Step 5 backend band가 겹쳐 하단 패널이 잘려 보였음 → 1600×1080 + 글씨 크기 / 패널 padding / level meter 두께 키움 + `ScrollViewer` 적용. 두 source meter가 한눈에 보이도록 개선.

재현 가능 절차 (사용자가 본 라운드에 실제 돌린 순서):

```powershell
# 1) backend
cd server
npm run db:migrate:up    # 필요 시 (Step 5는 migration 무변경)
npm run db:seed
npm run dev              # :32173
...
본 E2E 통과로 `PHASE_9_MASTER.md` Step 5 체크박스를 `[x]`로 갱신했다 (Plan §8 정책).
...
| backend auth 실패 → friendly UI 메시지 | OK — manual E2E 도중 잘못된 token + 잘못된 비밀번호 케이스 surface, `OnSocketTransportFailed` / `DesktopAuthClient` friendly message가 `LastRealtimeError`로 표시됨 |

---

## 11. Known Limitations / Risks

1. **WPF `PasswordBox` 바인딩 미사용.** `LoginPassword`를 `TextBox`로 입력받는다 (PasswordBox는 binding 안 됨). 화면에 평문으로 노출되지만 UI 입력 직후 메모리로 옮기고 즉시 비움. dev fallback이라 product polish 단계(Step 7)에서 PasswordBox + behavior로 교체 권장.
2. **재연결 자동화 없음.** Plan §5.3 그대로 — `Reconnection = false`로 두고 사용자가 명시적으로 "연결 끊기" → 재연결 흐름 사용. Step 7 hardening에서 짧은 메모리-only 재연결 버퍼 검토.
3. **MFA UX 미지원.** `/auth/login`이 HTTP 202를 반환하면 unsupported 안내만 노출하고 paste-token으로 우회. 풀 desktop MFA UX는 Step 7 이후 (별 step).
4. **Desktop-side raw audio sentinel 자동 테스트 부재.** byte[] reference가 sink → EmitAsync 1 경로만 거치는 게 정적 분석으로 명확하지만, refactor 회귀 방지용 desktop xunit 테스트는 본 라운드 미추가. Step 7 hardening 후보.
5. **`SocketIOClient` reconnection / heartbeat 정책.** 기본 옵션을 그대로 쓰지만 backend `socket.io@4.x` 의 ping/pong 간격과 일치하는지 명시 검증은 manual e2e 도중 별도 미관측 — 짧은 세션은 안정적이었으나 5분 이상 장기 세션은 Step 7 hardening 시점에 별도 stress test 필요.
6. **MainWindowViewModel의 dirty 보존.** Step 4 한국어화 + ProgressBar `Mode=OneWay` fix(Codex blocker)를 그대로 두고 위에만 추가. 단 동일 파일 +200줄 가까이 커졌다 — Step 6 / Step 7 시점에 `RealtimeViewModel`로 분리 권장.
7. **중도 disconnect stress는 별도 미검증.** 코드상 active call 중 disconnect / transport failure는 `CleanupCallSession()` + `StopCapture()`로 fail-closed 처리한다. 다만 manual E2E는 정상 종료 중심으로 수행했고, 네트워크 중단 / 재연결 반복 / 장기 세션 stress는 Step 7 hardening에서 별도 검증해야 한다.
8. **EngineIO V4 강제.** SocketIOClient 3.1.2의 `EngineIO.V4`로 hard-coded. backend가 v3 fallback이 필요해지면 옵션 추가 필요. 본 backend는 socket.io 4.x고정이므로 무해.
9. **`AudioRuntimeErrorCode` casing 비대칭.** backend 정의가 대문자 4종(audio fail-closed) + 소문자 5종(call/connection lifecycle)로 혼합. 향후 wire 변경 시 `server/src/types/wsAudio.ts` + `platform/types/wsAudio.js` + desktop `OnSocketRuntimeError` 분기까지 3-way로 lockstep 갱신해야 한다. Step 7 hardening 단계에서 case-insensitive whitelist로 정규화하면 desktop 측 휴먼 에러 방지에 도움이 될 수 있음 (현재는 명시적 case-sensitive로 두는 게 backend 정본과 일치).
...
- [x] manual e2e (E1~E11) — 사용자 머신에서 PASS (§9).
- [x] `PHASE_9_MASTER.md` Step 5 체크박스 `[x]`로 갱신.
