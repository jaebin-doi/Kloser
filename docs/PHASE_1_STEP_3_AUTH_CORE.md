...
- [ ] 3. env 계약 추가 (`JWT_SECRET`, `ACCESS_TOKEN_TTL`, `REFRESH_TOKEN_TTL`, `REFRESH_GRACE_WINDOW`, cookie 설정 — `JWT_SECRET` 누락 시 boot fail-fast)
...
4. row가 revoked 상태:
   - **grace window 내** (`revoked_at >= now() - REFRESH_GRACE_WINDOW` AND `replaced_by_session_id IS NOT NULL`):
...
`REFRESH_GRACE_WINDOW`는 env로 노출, 기본값 `30s`. 너무 짧으면 정상 동시성을 reuse로 오인하고, 너무 길면 진짜 탈취 토큰의 활용 창이 늘어남.
...
| Refresh rotation grace window | §3 Rotation 참고 — `REFRESH_GRACE_WINDOW=30s`(기본). 정상 멀티탭 동시성을 false-positive family revoke로 보내지 않기 위해. env override 가능. |
